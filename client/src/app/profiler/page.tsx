...
export default function ProfilerPage() {
    ...
    const finalizeProfiling = () => {
        ...
        const avg: BodyLanguageMetrics = {
            ...
            neckStability: samples.reduce((s, m) => s + (m.neckStability || 1), 0) / samples.length,
            shoulderExpansion: samples.reduce((s, m) => s + (m.shoulderExpansion || 1), 0) / samples.length,
            currentNeckRatio: samples.reduce((s, m) => s + (m.currentNeckRatio || 0), 0) / samples.length,
            currentBreadthRatio: samples.reduce((s, m) => s + (m.currentBreadthRatio || 0), 0) / samples.length,
            ...
            handEnergy: Math.round(samples.reduce((s, m) => s + (m.handEnergy || 0), 0) / samples.length),
            handsHidden: samples.filter(m => m.handsHidden).length / samples.length > 0.5,
