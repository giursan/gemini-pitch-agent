...
const adk_live_session_1 = require("./adk-live-session");
const content_agent_1 = require("./content-agent");
const cv_evaluator_1 = require("./cv-evaluator");
// ── Coach Chat Prompt ───────────────────────────────────────────────────────────
...
class Orchestrator {
    ...
    constructor(config) {
        ...
        this.contentAgent = new content_agent_1.ContentAgent();
        this.cvEvaluator = new cv_evaluator_1.CvEvaluator();
    ...
    async start() {
        ...
        if (this.config.projectId) {
            ...
            if (this.config.materialsContext)
                console.log(`[Orchestrator] Materials context: ${this.config.materialsContext.length} chars`);
            if (this.config.tasksContext)
                console.log(`[Orchestrator] Tasks context loaded`);
        }
    ...
    handleCvTelemetry(telemetry) {
        ...
        const signals = this.cvEvaluator.evaluate(telemetry, this.config.agents);
    ...
    async runContentAnalysis() {
        if (this.transcriptBuffer.trim().length < 30)
            return;
        try {
            ...
            const materialsPrefix = this.config.materialsContext
                ...
                : '';
            const assessment = await this.contentAgent.analyzeTranscript(materialsPrefix + this.transcriptBuffer);
    ...
    forwardAudioToClient(base64Pcm) {
        this.sendToClient({
            type: 'audio_output',
            data: base64Pcm,
        });
...
function scoreFillerRate(fillersPerMin) {
