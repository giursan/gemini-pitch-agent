 ...
 */
function aggregateDeliveryReports(timeline: any[]): Record<string, any> {
    ...
    for (const r of reports) {
        ...
        if (r.data?.transcript) transcripts.push(r.data.transcript);
    }

    return {
        avgPacing: Math.round(totalPacing / reports.length),
        totalFillers,
        fillerBreakdown: fillerWords.reduce((acc: Record<string, number>, w: string) => {
            acc[w] = (acc[w] || 0) + 1;
            return acc;
        }, {}),
